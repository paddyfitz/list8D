...
class GeSHi {
     ...
     * @since 1.0.0
     */
    function parse_code ($code_start=null,$code_end=null) {
        // Start the timer
        ...
        if ($this->error) {
            ...
            $this->finalise($result,$code_start,$code_end);
            return $result;
        }
...
//        $result = substr($result, 1, -1);
        ...
        $this->finalise($endresult,$code_start,$code_end);
        return $endresult;
    }
     ...
     * @access private
     */
    function finalise(&$parsed_code,$code_start,$code_end) {
        ...
        /** NOTE: memorypeak #2 */
        $code = explode("\n", $parsed_code);
        $parsed_code = $this->header($code_start);
        ...
        if ($this->line_numbers != GESHI_NO_LINE_NUMBERS && $this->header_type != GESHI_HEADER_PRE_TABLE) {
        		...
        		if (isset($code_start) && isset($code_end)) {
        			...
        			$code = array_splice($code,$code_start-1,$code_end-1);
       			}
     ...
     * @access private
     */
    function header($start_code=null) {
        ...
        if (GESHI_HEADER_NONE == $this->header_type) {
            if ($this->line_numbers != GESHI_NO_LINE_NUMBERS) {
            	...
            	if (isset($start_code)) {
            		return "$header<ol start='$start_code' $attributes$ol_attributes>";
            	} else {
